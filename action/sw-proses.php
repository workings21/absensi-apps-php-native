...
switch (@$_GET['action']) {
  ...
  case 'present':
    ...
    $latitude_array = explode(",", $latitude);

    $lat = $latitude_array[0];
    $long = $latitude_array[1];
    ...
    $query_u = "SELECT employees.id,employees.employees_code,employees.employees_name,employees.shift_id,employees.building_id,shift.shift_id,shift.time_in,shift.time_out FROM employees,shift WHERE employees.shift_id=shift.shift_id AND employees.id='$row_user[id]'";
    $result_u = $connection->query($query_u);
    if ($result_u->num_rows > 0) {
      $row_u = $result_u->fetch_assoc();
      $query  = "SELECT * FROM building WHERE building_id='$row_u[building_id]'";
      $result = $connection->query($query);
      $row = $result->fetch_assoc();

      $lat_building = $row['lat'];
      $long_building = $row['long'];
      ...
      if ($result->num_rows > 0) {
        ...
        // Check distance location and radius
        $theta = $long - $long_building;
        $miles = (sin(deg2rad($lat)) * sin(deg2rad($lat_building))) + (cos(deg2rad($lat)) * cos(deg2rad($lat_building)) * cos(deg2rad($theta)));
        $miles = acos($miles);
        $miles = rad2deg($miles);
        ...
        $return_distance = compact('miles', 'feet', 'yards', 'kilometers', 'meters');
        // End Check distance location and radius

        // Jika jarak lebih dari 50 meter maka ...
        if ((int) $return_distance['meters'] >= 50) {
          // echo "Anda tidak sedang di wilayah kantor ! pastikan anda berapa di radius 50 meter dari titik kantor anda ! (". $return_distance['meters'] ." Meter, ". $return_distance['kilometers']." Kilometer)";
          echo "Anda tidak sedang di wilayah kantor ! pastikan anda berapa di radius 50 meter dari titik kantor anda !";
          die;
        }
  ...
  case 'history':
    ...
    if ($result_absen->num_rows > 0) {
      while ($row_absen = $result_absen->fetch_assoc()) {
        ...
        if ($row_absen['present_id'] == 3 && $row_absen['status_izin'] == 0) {
          $status_izin = '<small class="badge badge-danger">Menunggu Verifikasi</small>';
          $status = '';
        } elseif ($row_absen['present_id'] == 3 && $row_absen['status_izin'] == 1) {
          $status_izin = '<small class="badge badge-success">Terverifikasi</small>';
          $status = '';
        } else {
          $status_izin = '';
          $status = '';
        ...
        <tr>
            ...
            <td class="hidden-sm">' . $row_aa['present_name'] . '' . $status . '' . $information . '<br> <br>'.$status_izin.'</td>
