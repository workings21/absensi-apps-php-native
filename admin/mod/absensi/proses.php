...
if (empty($_SESSION['SESSION_USER']) && empty($_SESSION['SESSION_ID'])) {
  ...
  switch (@$_GET['action']) {
    ...
    case 'verif_izin':
      ...
      // $update = "UPDATE presence SET status_izin = 1 WHERE " . $_POST['id'];
      $update = "UPDATE `presence` SET `status_izin`='1' WHERE `presence_id` = ". $_POST['id'];
...
<table class="table table-bordered table-hover" id="swdatatable">
        ...
        for ($d = 1; $d <= $jumlahhari; $d++) {
          ...
          $query_absen = "SELECT presence_id,status_izin,presence_date,time_in,time_out,picture_in,picture_out,present_id,presence_address,information,TIMEDIFF(TIME(time_in),'$shift_time_in') AS selisih,if (time_in>'$shift_time_in','Telat',if(time_in='00:00:00','Tidak Masuk','Tepat Waktu')) AS status FROM presence WHERE $filter ORDER BY presence_id DESC";
          ...
          if ($row_absen['status_izin'] == 1 && $row_absen['present_id'] == 3) {
            $status_izin = '<span class="label label-success">Verifed</span>';
            $status_time_in = '';
          } elseif ($row_absen['status_izin'] == 0 && $row_absen['present_id'] == 3) {
            $status_izin = '<span class="label label-danger">Unverifed</span>';
            $button_verif_izin = '<button type="button" data-idverif='. $row_absen['presence_id'] .' class="btn btn-success btn-xs verif-izin">Verifikasi Izin </button>';
            $status_time_in = '';
          }
         ...
         <tr style="background:' . $background . ';color:' . $warna . '">
          ...
          echo '
            ...
            <td>
              ' . $status_hadir . ' ' . $status_time_in . '<br>' .  $status_izin . '<br>' . $row_absen['information'] . '
            </td>
  
            <td class="text-right">
            <button type="button" class="btn btn-warning btn-xs btn-modal enable-tooltip" title="Lokasi" data-latitude="' . $latitude . '" data-longitude="' . $longitude . '"><i class="fa fa-map-marker"></i> Lokasi</button>
            ' . $button_verif_izin . '
            </td>
          </tr>';
        }
