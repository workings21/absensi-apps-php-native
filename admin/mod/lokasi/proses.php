...
if (empty($_SESSION['SESSION_USER']) && empty($_SESSION['SESSION_ID'])) {
  ...
  switch (@$_GET['action']) {
    case 'add':
      ...
      if (empty($error)) {
        $add = "INSERT INTO `building`(`code`, `name`, `address`, `building_scanner`, `lat`, `long`) VALUES ('$code','$name','$address','','$latitude','$longitude')";
...
---------------------------------*/
    case 'update':
      ...
      if (empty($error)) {
        // $update = "UPDATE building SET name='$name', address='$address', lat='$latitude', long='$longitude' WHERE building_id='$id'";
        $update = "UPDATE `building` SET `name`='$name',`address`='$address',`lat`='$latitude',`long`='$longitude' WHERE `building_id` = $id";
